  ...
  return (
    <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <nav className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400">
        ...
        >
          {lang === 'bn' ? 'সকল আর্টিকেল ও ব্লগ' : 'All Articles'}
        </button>
      ...
      <header className="bg-white dark:bg-slate-800/90 rounded-3xl p-6 sm:p-10 border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/80">
              ...
              <span>বাংলাদেশ ব্যাংক অনুমোদিত গাইড ২০২৬</span>
            </span>
          ...
          >
            ...
            <span>{copied ? (lang === 'bn' ? 'লিংক কপি হয়েছে!' : 'Link Copied!') : (lang === 'bn' ? 'শেয়ার করুন' : 'Share Article')}</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-700/60 text-xs text-slate-500 dark:text-slate-400">
          <div className="flex items-center gap-1.5">
            ...
            <span>লেখক: <strong className="text-slate-700 dark:text-slate-200">{article.author}</strong></span>
          </div>
          <div className="flex items-center gap-1.5">
            <Calendar className="w-3.5 h-3.5 text-slate-400" />
            <span>সর্বশেষ সংস্করণ: <strong className="text-emerald-700 dark:text-emerald-300">{CURRENT_DATA_VERSION_DATE}</strong></span>
          </div>
          <button
          ...
          >
            ...
            <span>ব্যাংকের মূল পেজ ও সকল ব্রাঞ্চ দেখুন ({branches.length})</span>
          </button>
        </div>
      </header>
      ...
      <section className="bg-emerald-900 text-white rounded-3xl p-6 sm:p-8 shadow-lg space-y-6">
        <h2 className="text-base sm:text-lg font-bold flex items-center gap-2 text-emerald-200">
          ...
          <span>{isRussian ? 'Основные реквизиты и коды банка (Quick Statistics)' : isHindi ? 'बैंक के महत्वपूर्ण कोड एवं आंकड़े (Quick Statistics)' : isBengali ? 'এক নজরে ব্যাংকের গুরুত্বপূর্ণ কোড ও তথ্য (Quick Statistics)' : 'Quick Bank Statistics & Verification'}</span>
        </h2>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 text-xs">
          {article.quick_stats.map((stat, i) => {
            const statLabel = isRussian && stat.label_ru ? stat.label_ru : isHindi && stat.label_hi ? stat.label_hi : isBengali && stat.label_bn ? stat.label_bn : stat.label;
            const statValue = isRussian && stat.value_ru ? stat.value_ru : isHindi && stat.value_hi ? stat.value_hi : isBengali && stat.value_bn ? stat.value_bn : stat.value;
            return (
      ...
      <div className="bg-white dark:bg-slate-800/90 rounded-3xl p-6 sm:p-10 border border-slate-200/80 dark:border-slate-700/80 shadow-xs space-y-10">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-bold text-emerald-900 dark:text-emerald-300 mb-1">
                {isRussian ? 'Краткое резюме руководства' : isHindi ? 'गाइड सारांश एवं अवलोकन' : isBengali ? 'গাইড ওভারভিউ ও ভূমিকা' : 'Guide Summary'}
              </h3>
        ...
        {article.sections.map((section, idx) => {
          const heading = isRussian && section.heading_ru ? section.heading_ru : isHindi && section.heading_hi ? section.heading_hi : isBengali && section.heading_bn ? section.heading_bn : section.heading;
          const content = isRussian && section.content_ru ? section.content_ru : isHindi && section.content_hi ? section.content_hi : isBengali && section.content_bn ? section.content_bn : section.content;
        ...
        <section className="pt-8 border-t border-slate-100 dark:border-slate-700/60 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span>{isRussian ? `Филиалы и отделения банка (${branches.length} отделений)` : isHindi ? `इस बैंक की प्रमुख शाखाएं एवं IFSC कोड्स (${branches.length} शाखाएं)` : isBengali ? `এই ব্যাংকের প্রধান শাখা ও রাউটিং কোডসমূহ (${branches.length}টি শাখা)` : `Key Bank Branches & Codes (${branches.length} branches)`}</span>
            </h2>
            <button
              onClick={() => onSelectBank(article.bank_id)}
              className="text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline cursor-pointer"
            >
              {isRussian ? `Все ${branches.length} отделений в справочнике →` : isHindi ? `सभी ${branches.length} शाखाएं देखें →` : isBengali ? `সকল ${branches.length}টি শাখা ডিরেক্টরিতে দেখুন →` : `View all ${branches.length} branches →`}
            </button>
          </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                    {isRussian && branch.name_ru ? branch.name_ru : isHindi && branch.name_hi ? branch.name_hi : isBengali && branch.name_bn ? branch.name_bn : branch.name}
                  </span>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-emerald-100/70 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 font-bold">
                    {branch.bik_code || branch.ifsc_code || branch.routing_number}
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                  {isRussian && branch.district_ru ? branch.district_ru : isHindi && branch.district_hi ? branch.district_hi : isBengali && branch.district_bn ? branch.district_bn : branch.district}, {isRussian && branch.division_ru ? branch.division_ru : isHindi && branch.division_hi ? branch.division_hi : isBengali && branch.division_bn ? branch.division_bn : branch.division}
                </p>
              </div>
            ))}
          </div>
        ...
        {article.faqs && article.faqs.length > 0 && (
          <section className="pt-8 border-t border-slate-100 dark:border-slate-700/60 space-y-4">
            <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span>{isRussian ? 'Часто задаваемые вопросы (FAQ)' : isHindi ? 'अक्सर पूछे जाने वाले प्रश्न (Frequently Asked Questions)' : isBengali ? 'সাধারণ জিজ্ঞাসিত প্রশ্নাবলী (Frequently Asked Questions)' : 'Frequently Asked Questions'}</span>
            </h2>

            <div className="space-y-3">
              {article.faqs.map((faq, i) => {
                const q = isRussian && faq.question_ru ? faq.question_ru : isHindi && faq.question_hi ? faq.question_hi : isBengali && faq.question_bn ? faq.question_bn : faq.question;
                const a = isRussian && faq.answer_ru ? faq.answer_ru : isHindi && faq.answer_hi ? faq.answer_hi : isBengali && faq.answer_bn ? faq.answer_bn : faq.answer;
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        >
          ...
          <span>{lang === 'bn' ? 'সকল ব্যাংকের আর্টিকেল তালিকায় ফিরে যান' : 'Back to All Articles'}</span>
        </button>
