...
export const BranchCard: React.FC<BranchCardProps> = ({
  ...
  const isSingapore = branch.country === 'sg' || !!branch.clearing_code;
  ...
  const shareText = isUS
    ...
    : isSingapore
    ? `Singapore Clearing: ${branch.clearing_code || branch.routing_number} | Bank: ${branch.bank_code} | Branch: ${branch.branch_code} | Singapore`
    : isGermany
    ? `Bankleitzahl (BLZ): ${branch.blz || branch.routing_number} | IBAN: ${branch.iban_sample || 'DE...'} | SWIFT: ${branch.swift_code || 'N/A'} | Germany`
    : isRussia
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-700/80 text-slate-600 dark:text-slate-300 whitespace-nowrap">
              {isUS ? '🇺🇸 US' : isUK ? '🇬🇧 UK' : isCanada ? '🇨🇦 CA' : isAustralia ? '🇦🇺 AU' : isUAE ? '🇦🇪 AE' : isSingapore ? '🇸🇬 SG' : isGermany ? '🇩🇪 DE' : isRussia ? '🇷🇺 RU' : isIndia ? '🇮🇳 IN' : '🇧🇩 BD'}
            </span>
            <ShareButton
              title={shareTitle}
              text={shareText}
          ...
          <div className="bg-emerald-50/70 dark:bg-emerald-950/40 p-3 sm:p-3.5 rounded-2xl border border-emerald-200/70 dark:border-emerald-800/50 mb-3 space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-1">
              <div className="flex items-center space-x-1.5 min-w-0">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">
                  {lang === 'bn' ? 'অস্ট্রেলিয়ান BSB কোড:' : lang === 'hi' ? 'ऑस्ट्रेलिया BSB कोड:' : lang === 'ru' ? 'Австралийский BSB код:' : 'Australian BSB Code:'}
                </span>
              </div>
              <span className="text-[10px] font-bold text-emerald-800 dark:text-emerald-300 uppercase">
                AusPayNet • NPP Osko
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 bg-white dark:bg-slate-800 px-3 py-2 rounded-xl border border-emerald-200/80 dark:border-emerald-700/60 shadow-2xs">
              <span className="font-mono text-base sm:text-lg font-extrabold text-slate-900 dark:text-white tracking-wider break-all min-w-0">
                {branch.bsb_code ? (branch.bsb_code.includes('-') ? branch.bsb_code : `${branch.bsb_code.slice(0, 3)}-${branch.bsb_code.slice(3)}`) : branch.routing_number}
              </span>
              <CopyButton
          ...
          <div className="bg-emerald-50/70 dark:bg-emerald-950/40 p-3 sm:p-3.5 rounded-2xl border border-emerald-200/70 dark:border-emerald-800/50 mb-3 space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-1">
              <div className="flex items-center space-x-1.5 min-w-0">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">
                  {lang === 'bn' ? 'BLZ (ব্যাংক লাইটজাল):' : lang === 'ru' ? 'БЛЦ (Bankleitzahl):' : 'German BLZ (Bankleitzahl):'}
                </span>
              </div>
              <span className="text-[10px] font-bold text-emerald-800 dark:text-emerald-300 uppercase">
                Bundesbank • SEPA
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 bg-white dark:bg-slate-800 px-3 py-2 rounded-xl border border-emerald-200/80 dark:border-emerald-700/60 shadow-2xs">
              <span className="font-mono text-base sm:text-lg font-extrabold text-slate-900 dark:text-white tracking-wider break-all min-w-0">
                {branch.blz || branch.routing_number}
              </span>
              <CopyButton
                textToCopy={branch.blz || branch.routing_number}
                label={t.copy}
                size="sm"
                lang={lang}
                className="shrink-0"
            ...
            {branch.iban_sample && (
              <div className="flex items-center justify-between gap-2 text-[11px] pt-1 border-t border-emerald-200/50 dark:border-emerald-800/40">
                <span className="text-slate-600 dark:text-slate-400 font-medium truncate">
                  Sample IBAN: <span className="font-mono font-bold text-slate-800 dark:text-slate-200">{branch.iban_sample}</span>
                </span>
                <CopyButton textToCopy={branch.iban_sample} label={t.copy} size="sm" lang={lang} className="scale-90" />
              </div>
            )}
          </div>
        ) : isRussia ? (
